...
class RNNModel(nn.Module,OpenAttack.Classifier):
    ...
    def forward(self, x):
        ...
        print("h0 size: "+ str(h0.size())+", x size: " + str(x.size()))
...
def main():
    ...
    x_test = torch.load('../Adversarial-Attack-on-recurrent-neural-network/x_test_tomita6.pt').numpy()
    y_test = torch.load('../Adversarial-Attack-on-recurrent-neural-network/y_test_tomita6.pt').numpy()
    x_train = torch.load('../Adversarial-Attack-on-recurrent-neural-network/x_train_tomita6.pt').numpy()
    y_train = torch.load('../Adversarial-Attack-on-recurrent-neural-network/y_train_tomita6.pt').numpy()
    dataset = []
    for i in range(len(x_test)):
        this_dict = {'x': list(x_test[i]),'y': int(y_test[i])}
